...
// "Equal to" operator for ClockParameters
bool MCP2221::ClockParameters::operator ==(const MCP2221::ClockParameters &other) const
{
    return div == other.div && dc == other.dc;
}

// "Not equal to" operator for ClockParameters
bool MCP2221::ClockParameters::operator !=(const MCP2221::ClockParameters &other) const
{
    return !(operator ==(other));
}
...
bool MCP2221::USBParameters::operator ==(const MCP2221::USBParameters &other) const
{
    return serialen == other.serialen && vid == other.vid && pid == other.pid && maxpow == other.maxpow && powmode == other.powmode && rmwakeup == other.rmwakeup;
}
...
{
    return clock == other.clock && intr == other.intr && adc == other.adc && dac == other.dac && usb == other.usb;
}
...
MCP2221::ChipSettings MCP2221::getChipSettings(int &errcnt, QString &errstr)
{
    ...
    settings.clock.div = static_cast<quint8>(0x07 & response.at(5));                  // Clock output divider corresponds to bits 2:0 of byte 5
    settings.clock.dc = static_cast<quint8>(0x03 & response.at(5) >> 3);              // Clock output duty cycle corresponds to bits 4:3 of byte 5
    ...
    settings.usb.vid = static_cast<quint16>(response.at(9) << 8 | response.at(8));    // Vendor ID corresponds to bytes 8 and 9 (little-endian conversion)
    settings.usb.pid = static_cast<quint32>(response.at(11) << 8 | response.at(10));  // Product ID corresponds to bytes 10 and 11 (little-endian conversion)
    settings.usb.maxpow = response.at(13);                                            // Maximum consumption current corresponds to byte 13
    settings.usb.powmode = (0x40 & response.at(12)) != 0x00;                          // Power mode corresponds to bit 6 of byte 12
    settings.usb.rmwakeup = (0x20 & response.at(12)) != 0x00;                         // Remote wake-up capability corresponds to bit 5 of byte 12
    return settings;
}
...
quint8 MCP2221::writeChipSettings(const ChipSettings &settings, SecurityOptions &options, const QString &password, int &errcnt, QString &errstr)
{
    ...
    if (passwordLength > static_cast<int>(PASSWORD_MAXLEN)) {
        ...
        command[3] = static_cast<quint8>((0x03 & settings.clock.dc) << 3 | (0x07 & settings.clock.div));                                                      // Clock output parameters
